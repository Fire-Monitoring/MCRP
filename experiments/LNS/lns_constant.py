...
file_name = f'constant.txt'

DRONE_SPEED[0] = DRONE_SPEED[0]
res_dict = dict()
duration = max(MAXIMUM_SORTIE_TIME[0], TIME_TO_MAX_RISK)*5
print("[", end="")
...
for x in X:
            res = simulate_path(width_of_map_for_dynamic=15, map_name=x, n_steps_or_sorties=1, algorithm_name=alg_name,
                          arguments=["lns", "short", "static", 2], by_steps=False, duration=duration, return_results=True)
            print(f"{res}, ", end="")
            with open(file_name, 'a') as f:
                f.write(str(res))
                f.write(", ")
